...
func (fw *FileWatcher) handleEvent(fileEvent fsnotify.Event) {
	// Log the event with source attribution
	switch {
	...
	case fileEvent.Op&fsnotify.Chmod == fsnotify.Chmod:
		// Ignore chmod events - don't trigger reconciliation
		fw.logger.Debug("ignoring chmod event", "path", fileEvent.Name, "op", "CHMOD")
		return
	default:
	...
	fw.debounceTimer = time.AfterFunc(fw.debounceDuration, func() {
