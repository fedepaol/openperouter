...
// given namespace.
func setupVeth(ctx context.Context, vethNames VethNames, targetNS netns.NsHandle) error {
...
func setupNamespaceVeth(ctx context.Context, vethNames VethNames, namespace string) error {
	slog.DebugContext(ctx, "setting up veth", "hostSide", vethNames.HostSide, "nsSide", vethNames.NamespaceSide)
	defer slog.DebugContext(ctx, "end setting up VNI", "hostSide", vethNames.HostSide, "nsSide", vethNames.NamespaceSide)
	ns, err := netns.GetFromName(namespace)
	if err != nil {
		return fmt.Errorf("SetupVNI: Failed to get network namespace %s: %w", namespace, err)
	}
	defer func() {
	...
	if err := setupVeth(ctx, vethNames, ns); err != nil {
		return err
	}
