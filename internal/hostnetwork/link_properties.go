...
func interfaceHasIP(link netlink.Link, address string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("interfaceHasIP: failed to parse address %s for interface %s", address, link.Attrs().Name)
	}
	addresses, err := netlink.AddrList(link, netlink.FAMILY_ALL)
	if err != nil {
		return false, fmt.Errorf("interfaceHasIP: failed to list addresses for interface %s", link.Attrs().Name)
	}
