...
echo "Host network namespace: ${host_netns}"
echo "Host default routes:"
...
for c in leafA leafB leafSRV6 hostA_red hostA_blue hostA_default hostB_red hostB_blue hostSRV6_red hostSRV6_blue; do
    ...
    if container_pid=$(timeout 10s ${CLI} inspect --format '{{.State.Pid}}' "${container}"); then
        container_netns=$(readlink "/proc/${container_pid}/ns/net")
    else
        rc=$?
        echo "Could not inspect ${container} network namespace (exit code ${rc})" >&2
        exit "${rc}"
    fi
    echo "${container} network namespace: ${container_netns} (PID ${container_pid})"
    if [ "${container_netns}" = "${host_netns}" ]; then
        echo "Refusing to run ${container}/setup.sh in the host network namespace" >&2
        exit 1
    fi
    echo "${container} default routes before setup:"
    nsenter --target "${container_pid}" --net ip -4 route show default
    nsenter --target "${container_pid}" --net ip -6 route show default
    ...
    if timeout 10s ${CLI} exec "${container}" test -f /setup.sh 2>/dev/null; then
        ...
        echo "${container} default routes after setup:"
        nsenter --target "${container_pid}" --net ip -4 route show default
        nsenter --target "${container_pid}" --net ip -6 route show default
    else
        rc=$?
